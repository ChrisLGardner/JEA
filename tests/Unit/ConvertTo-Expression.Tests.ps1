...
InModuleScope JeaDsc {
	...
	Describe 'ConvertTo-Expression' {
		...
		$Version = [System.Version]'1.2.3.4'
		$Guid = [guid]'5f167621-6abe-4153-a26c-f643e1716720'
		$TimeSpan = New-TimeSpan -Hours 1 -Minutes 25
		$DateTime = Get-Date
		Mock Get-Date -ParameterFilter {
			$null -eq $Date
		} {
			$DateTime
		}
		...
		Context 'custom object' {
			...
			$null = $DataTable.Columns.Add((New-Object -TypeName Data.DataColumn -ArgumentList 'Column1'), [string])
			$null = $DataTable.Columns.Add((New-Object -TypeName Data.DataColumn -ArgumentList 'Column2'), [int])
...
"@
		...
		Context 'Credentials and SecureString' {
			...
			$Credential = New-Object -TypeName PSCredential -ArgumentList $Username, $SecureString
